...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Setup midnight reset listener
    async def check_midnight_reset(now):
        """Check if we need to reset items at midnight."""
        if entry.options.get("auto_uncheck", False):
            await data_store.reset_completed_items()
            await coordinator.async_refresh()

    # Listen for midnight (00:00:00)
    from homeassistant.helpers.event import async_track_time_change
    entry.async_on_unload(
        async_track_time_change(hass, check_midnight_reset, hour=0, minute=0, second=0)
    )
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    hass.data[DOMAIN][entry.entry_id] = {
        "coordinator": coordinator,
        "data": data_store,
    } # Closing bracket added here to fix syntax
    hass.services.async_register(
        DOMAIN,
        "remove_item",
        remove_item,
...
class WeekliesData:
    ...
    async def reset_completed_items(self) -> None:
        """Reset all completed items to 'needs_action'."""
        changed = False
